...
const MessageBox = async({ message, className }: { message: Message, className: string }) =>{
  ...
  const renderContent = () => {
    switch (message.type_message) {
      ...
      default:
        return <p className={`text-gray-800 bg-white-50  p-2 w-fit ${message.sender.id == JSON.parse(.get("userInfo")?.value || "")?.id ? "" : "rounded-b-[8px] rounded-tr-[8px]"}`}>{message.content}</p>;
    }
  };
