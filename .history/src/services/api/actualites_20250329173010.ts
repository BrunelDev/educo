...
import {
  ...
  ActualiteSchema,
  PaginatedActualite,
  PaginatedActualiteSchema,
} from "./schemas/actualite";
...
export class ActualitesService {
  ...
  async getActualites(page?: number): Promise<PaginatedActualite> {
    try {
      ...
      logger.error("Erreur lors de la récupération des actualités", {
        error,
        page,
      });
      throw error;
  ...
  async getActualite(id: number): Promise<Actualite> {
    try {
      ...
      logger.error("Erreur lors de la récupération de l'actualité", {
        error,
        id,
      });
      throw error;
  ...
  async updateActualite(
    ...
    try {
      ...
      logger.error("Erreur lors de la mise à jour de l'actualité", {
        error,
        id,
      });
      throw error;
  ...
  async deleteActualite(id: number): Promise<void> {
    try {
      ...
      logger.error("Erreur lors de la suppression de l'actualité", {
        error,
        id,
      });
      throw error;
