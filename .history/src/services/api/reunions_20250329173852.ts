...
export class ReunionsService {
  ...
  async getReunions(page?: number): Promise<PaginatedReunion> {
    try {
      ...
      logger.error("Erreur lors de la récupération des réunions", {
        error,
        page,
      });
      throw error;
  ...
  async getReunion(id: number): Promise<Reunion> {
    try {
      const response = await this.client.request<Reunion>(
        `/api/reunions/${id}/`
      );
      ...
      logger.error("Erreur lors de la récupération de la réunion", {
        error,
        id,
      });
      throw error;
  ...
  async updateReunion(id: number, reunion: Partial<Reunion>): Promise<Reunion> {
    try {
      ...
      logger.error("Erreur lors de la mise à jour de la réunion", {
        error,
        id,
      });
      throw error;
