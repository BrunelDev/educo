...
const NotificationItem = ({notification} : {notification :Notification}) => {
    return (
      <div className="relative border-b border-dashed hover:px-3 group hover:bg-white-100 rounded-lg py-1 duration-200 w-full">
        {notification.est_lu ? (<div className="absolute top-1/2 right-0 rounded-full bg-coral-500 w-1 h-1 group-hover:right-3 duration-200"></div>) : null}
                <div className="flex w-full gap-2">
                    ...
                    <div className="flex justify-between w-[288px]">
                        <h3 className="text-sm text-white-800 font-semibold">{notification.titre}</h3>
                        <h6 className="text-xs text-gray-600">{new Date(notification.date_creation).getHours()+":"+new Date(notification.date_creation).getMinutes()}</h6>
                    </div>
                </div>
                
            <p className="text-sm truncate">{notification.contenu_associe_info.str_representation}</p>
