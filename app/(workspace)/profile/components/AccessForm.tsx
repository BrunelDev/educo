...
const formSchema = z.object({
  memberId: z.number(), // 0 means no member selected
  role: z.string(),
}).refine(data => {
  if (data.memberId !== 0) {
...
export function AccessForm() {
  ...
  const [memberRoles, setMemberRoles] = useState<{ [memberId: number]: string }>({});
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      console.log("Selected member ID:", values.memberId, "Selected Role:", values.role);
  ...
  const handleRoleChange = (memberId: number, newRole: string) => {
    ...
    if (form.getValues("memberId") === memberId) {
      form.setValue("role", newRole, { shouldValidate: true });
    }
  };

  const handleCheckboxChange = (checked: boolean, memberId: number) => {
    const currentFormMemberId = form.getValues("memberId");
    if (checked) {
      // Checkbox is being checked
      form.setValue("memberId", memberId, { shouldValidate: true });
      form.setValue("role", memberRoles[memberId] || "", { shouldValidate: true });
    } else if (currentFormMemberId === memberId) {
      // Checkbox is being unchecked, and it was the one corresponding to form's memberId
      form.setValue("memberId", 0, { shouldValidate: true });
            ...
            <FormItem>
              ...
              <FormControl>
                <ScrollArea className="h-[200px] border rounded-md p-2">
                  {members.length > 0 ? (
                    members.map((member) => (
                      <div key={member.id} className="flex flex-col sm:flex-row sm:justify-between sm:items-center py-2 px-1 gap-2">
                        <div className="flex gap-3 items-center flex-grow w-full sm:w-auto">
                          ...
                          <span className="text-sm truncate" title={`${member.first_name} ${member.last_name} (${member.email})`}>
                        ...
                        <div className="flex items-center space-x-3 w-full sm:w-auto">
                          ...
                          <Checkbox
                            checked={field.value === member.id} // field.value is form.getValues("memberId")
                            onCheckedChange={(isChecked) => 
                              handleCheckboxChange(isChecked as boolean, member.id)
                            }
                          />
                        </div>
                      </div>
